...
class CurrentWeather {
    ...
    async get(location, aqi = false) {
        ...
        const url = `http://api.weatherapi.com/v1/current.json?key=${weatherClient.options.apiKey}&q=${location}&aqi=${aqi}&lang=${weatherClient.options.language}`

        const response = await axios.get(url)
            .catch(error => Util.handleError(error))
